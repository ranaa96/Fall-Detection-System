...
int temp = 0 ;
char t[2] = "", a[2] = "";
char first[] = "http://192.168.1.6/apiconn.php?sensor=";
char second[] = "&button=";
int x ;
...
void loop()
{
  ...
  if(act.isFreeFall || temp){
     if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
      ...
      if (act.isFreeFall){
        http.begin("http://192.168.1.6/apiconn.php?sensor=1&button=0");
      }else if (temp){
        http.begin("http://192.168.1.6/apiconn.php?sensor=0&button=1");
      }
      else{
